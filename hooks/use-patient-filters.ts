...
export function usePatientFilters() {
	...
	const activeFilters = useMemo(
		...
		[sortBy, sortOrder, medicalIssueFilters]
	);
	...
	const handleMedicalIssueFilterAdd = useCallback((issue: string) => {
		setMedicalIssueFilters((prev) => (prev.includes(issue) ? prev : [...prev, issue]));
	}, []);

	const handleMedicalIssueFilterRemove = useCallback((issue: string) => {
		setMedicalIssueFilters((prev) => prev.filter((f) => f !== issue));
	}, []);
